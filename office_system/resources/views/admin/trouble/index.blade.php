                        ...
                        <tbody>
                            @foreach ($troubles as $trouble)
                                <tr class="text-center align-middle">
                                    ...
                                    <td class="text-sm text-nowrap">{{!empty($trouble->corresponding_user_id) ? $trouble->user->name : ''}}</td>
                                    <td class="text-sm text-nowrap">{{!empty($trouble->corresponding_limit) ? \Carbon\Carbon::parse($trouble->corresponding_limit)->format('Y年m月d日') : ''}}</td>
                                    ...
                                    <td class="text-sm text-nowrap">{{config('const.trouble.register_type')[$trouble->register_type]}}</td>
                                    <td class="text-sm text-nowrap">{{\Carbon\Carbon::parse($trouble->created_at)->format('Y年m月d日')}}</td>
                                    <td class="text-sm text-nowrap">{{!empty($trouble->updated_at) ? \Carbon\Carbon::parse($trouble->updated_at)->format('Y年m月d日') : ''}}</td>
                                    ...
                                    <td class="text-sm text-nowrap">
                                        <form action="{{route('admin.trouble.delete', ['id' => $trouble->id])}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger btn-sm submit" onclick="return confirm('本当に削除しますか？');">削除</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
