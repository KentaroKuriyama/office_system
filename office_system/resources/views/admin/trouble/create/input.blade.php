                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="function" class="form-control">
                                    @foreach (config('const.trouble.function') as $key => $value)
                                        <option value="{{ $key }}" {{ old('function') == $key ? 'selected' : '' }}>
                                            {{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="cause_type" class="form-control">
                                    ...
                                    @foreach (config('const.trouble.cause_type') as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('cause_type') == $key ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="cause_process" class="form-control">
                                    <option value="" {{ old('cause_process') == '' ? 'selected' : '' }}>選択しない
                                    </option>
                                    @foreach (config('const.trouble.cause_process') as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('cause_process') == $key ? 'selected' : '' }}>{{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="corresponding_user_id" class="form-control">
                                    <option value="" {{ old('corresponding_user_id') == '' ? 'selected' : '' }}>
                                        まだ決まっていない</option>
                                    @foreach ($admins as $admin)
                                        <option value="{{ $admin->id }}"
                                            {{ old('corresponding_user_id') == $admin->id ? 'selected' : '' }}>
                                            {{ $admin->name }}</option>
                                    @endforeach
                                </select>
                            </div>
