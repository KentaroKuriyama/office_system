...
@section('title', config('app.name') . '| 障害登録')
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="function" class="form-control">
                                    @foreach (config('const.trouble.function') as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('function', $trouble) == $key ? 'selected' : '' }}>{{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="cause_type" class="form-control">
                                    <option value="" {{ old('cause_type', $trouble) == '' ? 'selected' : '' }}>選択しない
                                    </option>
                                    @foreach (config('const.trouble.cause_type') as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('cause_type', $trouble) == $key ? 'selected' : '' }}>
                                            {{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="cause_process" class="form-control">
                                    <option value="" {{ old('cause_process', $trouble) == '' ? 'selected' : '' }}>
                                        選択しない</option>
                                    @foreach (config('const.trouble.cause_process') as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('cause_process', $trouble) == $key ? 'selected' : '' }}>
                                            {{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="corresponding_user_id" class="form-control">
                                    <option value=""
                                        {{ old('corresponding_user_id', $trouble) == '' ? 'selected' : '' }}>まだ決まっていない
                                    </option>
                                    @foreach ($admins as $admin)
                                        <option value="{{ $admin->id }}"
                                            {{ old('corresponding_user_id', $trouble) == $admin->id ? 'selected' : '' }}>
                                            {{ $admin->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                @error('corresponding_limit')
                                    <b><span class="text-danger">・{{ $message }}</span></b>
                                @enderror
                                <input type="date" class="form-control" name="corresponding_limit"
                                    value="{{ old('corresponding_limit', $trouble) }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="priority" class="form-control">
                                    @foreach (config('const.trouble.priority') as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('priority', $trouble) == $key ? 'selected' : '' }}>{{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="status" class="form-control">
                                    @foreach (config('const.trouble.status') as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('status', $trouble) == $key ? 'selected' : '' }}>{{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
